...
<%@ page import="dto.ReviewDetailDisplayDTO"%> <%-- 이제 이 DTO를 사용합니다 --%>
<%@ page import="model.ContentBlock"%> <%-- ContentBlock 모델 임포트 --%>
...
<body>
    ...
    <div class="container">
        ...
        <div class="review-header">
            <img src="${fn:escapeXml(reviewDetail.bookCoverImageUrl)}" alt="${fn:escapeXml(reviewDetail.bookTitle)} 표지"
            ...
            <div class="book-meta">
                ...
                <div class="review-info">
                    <span>작성자: ${fn:escapeXml(reviewDetail.userId)}</span> <%-- 이 부분은 user_id(int)를 표시하므로, 나중에 사용자 닉네임/이름을 가져와 표시하도록 변경해야 합니다. --%>
                    <div class="user-rating">
                        <c:forEach begin="1" end="${reviewDetail.rating}">★</c:forEach>
                        <c:forEach begin="1" end="${5 - reviewDetail.rating}">☆</c:forEach>
                    </div>
                </div>
                <p class="created-at">작성일: ${reviewDetail.createdAt.toLocalDate()}</p>
            </div>
        </div>
        ...
        <div class="review-actions">
            <%-- 로그인한 사용자가 이 리뷰의 작성자인 경우에만 수정/삭제 버튼 표시 --%>
            <c:if test="${not empty loggedInUser and loggedInUser.userId == reviewDetail.userId}">
                <a href="<%=contextPath%>/review/edit?id=${reviewDetail.reviewId}" class="action-btn edit">수정</a>
                <a href="<%=contextPath%>/deleteReview?id=${reviewDetail.reviewId}" class="action-btn delete" onclick="return confirm('정말로 이 리뷰를 삭제하시겠습니까?');">삭제</a>
            </c:if>
